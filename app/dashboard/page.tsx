...
// ✅ Tipo para el usuario de sesión
type UsuarioSesion = {
  nombre: string;
  cargo: string;
  sexo: string;
...
export default function DashboardPage() {
  ...
  // ✅ Estado del usuario real de sesión
  const [userSession, setUserSession] = useState<UsuarioSesion>({
    nombre: "...",
    cargo: "Sistema CAZ - Recursos Humanos",
    sexo: "M",
  });
  ...
  const [formData, setFormData] = useState({
    dni: "", nro_legajo: "", apellido: "", nombre: "",
    cuil: "", estado: "Activo", cargo: "", fecha_ingreso: ""
  });

  useEffect(() => {
    cargarLegajos();
    cargarUsuarioSesion(); // ✅ Cargamos el usuario real al montar
  }, []);

  // ✅ Función que usa el Display Name de Supabase Auth directamente
  async function cargarUsuarioSesion() {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session?.user) return;

      // El Display Name está en user_metadata.full_name o directamente en la sesión
      const displayName =
        session.user.user_metadata?.display_name ||
        session.user.email?.split("@")[0].toUpperCase() ||
        "USUARIO";

      // ✅ Detección de género por el primer nombre
      // Tomamos la primera palabra del display name
      const primerNombre = displayName.trim().split(" ")[0].toLowerCase();
      // Si el primer nombre termina en "a" (ej: "Maria", "Laura", "Ana") → femenino
      const esFemenino = primerNombre.endsWith("a") && primerNombre.length > 2;
  ...
  const card: React.CSSProperties = {
    ...
    background: "rgba(0,0,0,0.45)", backdropFilter: "blur(12px)", overflow: "hidden"
  };
  const inputBase: React.CSSProperties = {
    padding: "12px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.14)",
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 30 }}>
          ...
          <button onClick={() => setMostrarModal(true)}
            style={{ padding: "14px 28px", borderRadius: 14, border: "none", background: "#2563eb", color: "white", fontWeight: 800, cursor: "pointer" }}>
            ➕ Nuevo Legajo
          </button>
        </div>

        {/* ✅ PERFIL CON NOMBRE REAL DEL USUARIO */}
        <div style={{ ...card, padding: "15px 25px", marginBottom: 20, display: "flex", alignItems: "center", gap: 20, borderLeft: "4px solid #2563eb" }}>
          ...
          <div>
            {/* ✅ Saludo con género correcto */}
            <div style={{ fontSize: 18, fontWeight: 800, color: "#4ade80" }}>
              {userSession.sexo === "F" ? "Bienvenida" : "Bienvenido"}, {userSession.nombre}
            </div>
            <div style={{ fontSize: 13, opacity: 0.7 }}>
              {userSession.cargo} • Municipalidad de Salta
            </div>
          </div>
        </div>
        ...
        <div style={{ ...card, padding: 20, display: "grid", gridTemplateColumns: "1fr 280px 140px", gap: 15, alignItems: "end", marginBottom: 20 }}>
          <div>
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="DNI, Apellido o Legajo..."
              style={{ ...inputBase, width: "100%" }} />
          </div>
          <div>
            <label style={{ fontSize: 12, opacity: 0.7, marginBottom: 5, display: "block" }}>Estado</label>
            <select value={fEstado} onChange={(e) => setFEstado(e.target.value as "" | "Activo" | "Baja")}
              style={{ ...inputBase, width: "100%" }}>
          ...
          <button onClick={() => { setQ(""); setFEstado(""); }}
            style={{ ...inputBase, cursor: "pointer", fontWeight: 800 }}>
            Limpiar
          </button>
        </div>
        ...
        <div style={card}>
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            ...
            <tbody>
              {loading
                ? <tr><td colSpan={6} style={{ padding: 30, textAlign: "center" }}>Cargando...</td></tr>
                : filtrados.map((l) => (
                  <tr key={l.dni} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <td style={{ textAlign: "center" }}>
                      <button onClick={() => window.location.href = `/legajos/${l.dni}`}
                        style={{ ...inputBase, padding: "6px 15px", fontSize: 13, cursor: "pointer" }}>
                        Ver
                      </button>
                    </td>
                  </tr>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(5px)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 100 }}>
          <div style={{ ...card, width: "100%", maxWidth: 650, padding: 35, background: "#111", position: "relative" }}>
            <button onClick={() => setMostrarModal(false)}
              style={{ position: "absolute", top: 20, right: 20, background: "none", border: "none", color: "white", fontSize: 20, cursor: "pointer" }}>
              ✕
            </button>
            ...
            <form onSubmit={handleSubmitNuevo} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 15 }}>
              ...
              <button type="submit" disabled={submitting}
                style={{ gridColumn: "span 2", padding: 15, borderRadius: 12, border: "none", background: "#2563eb", color: "white", fontWeight: 800, cursor: "pointer", marginTop: 10 }}>
